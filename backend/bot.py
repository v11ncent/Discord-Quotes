# import libraries
import discord, aiohttp, datetime, json
...
async def send_data(data):
    async with aiohttp.ClientSession() as session:
        async with session.post('https://quotes-server0.herokuapp.com/', data=data) as resp:
            return await resp.text()
    
# discord bot token
client.run('ODMwMzIxNDYzNzk3MzUwNDEx.YHE_Hw.Y8FnjbJfrW-d7_OdpxBTUvWlNmc')
